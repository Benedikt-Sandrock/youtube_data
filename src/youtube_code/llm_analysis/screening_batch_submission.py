"""Shared core for submitting one production screening batch to Vertex AI.

Factored out of ``run_politics_screening_batch.py`` and
``run_longitudinal_screening_batch.py`` in Phase 4d, Schritt 6 (see
``.claude/plans/phase_4.md``), which were byte-for-byte identical except for
the ``ROUND_NUMBER`` default and a real column-name divergence between the
two screening pipelines (``time_period`` vs. ``interval_label``, both the
column that groups candidate videos by time window). Both differences are
now explicit parameters of :func:`submit_screening_batch` — ``round_number``
and ``period_column`` (plus ``period_noun`` for the human-readable label
used in the preflight printout) — instead of module-level constants that
existed twice with different values.
...
candidate IDs and their current labels are checked against the screening
state (``screening_state_store``, since Phase 4d).
"""
...
from youtube_code.step2_baseline_channels.longitudinal.prompts import (
...
# ============================================================
# FIXED PRODUCTION CONFIG (identical for both screening pipelines)
# ============================================================
...
def get_mode_settings(mode: str, period_column: str) -> ModeSettings:
...
def load_state() -> pd.DataFrame:
    ...
    state = normalize_ids(state, "screening state")
...
def load_and_validate_inputs(
    ...
    candidates = load_candidate_file(
        candidate_file, settings, round_number, period_column
    )
    state = load_state()
    expected = get_expected_state_rows(state, settings, round_number)
    require_exact_id_match(candidates, expected, settings, round_number)
...
def print_preflight(
    ...
    print(f"\nVideos by {period_noun}:")
    print(
        candidates[period_column]
        .value_counts()
        .sort_index()
        .to_string()
    )
...
def submit_screening_batch(
    *,
    ...
    """Validate candidates against the screening state and submit one
    production batch (title or description stage) to Vertex AI.

    ``period_column`` is the candidate-file column that groups videos by
    time window (``"time_period"`` for the politics pipeline,
    ``"interval_label"`` for the longitudinal pipeline); ``period_noun`` is
    its human-readable label used in the preflight printout ("period" /
    "interval").
    """
    settings = get_mode_settings(mode, period_column)
    ...
    candidates, candidate_file, empty_description_count = (
        load_and_validate_inputs(settings, round_number, period_column)
    )
    dataset_id = (
