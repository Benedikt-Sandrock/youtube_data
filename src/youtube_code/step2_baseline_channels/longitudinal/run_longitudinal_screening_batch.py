"""Submit one production longitudinal-screening batch to Vertex AI.
...
Only the config below differs from ``run_politics_screening_batch.py``:
this pipeline's candidate files use the ``interval_label`` column instead
of ``time_period``.
...
def main() -> None:
    submit_screening_batch(
        ...
        period_column="interval_label",
        period_noun="interval",
        mode=MODE,
        dry_run=DRY_RUN,
